	...
	mysqli_report(MYSQLI_REPORT_STRICT);
		...
		try 
		{
			...
			else
			{
				...
				$sql=mysql_query("SELECT `id`, `nazwa`, `opisk`, `status` FROM `Eprojekty` "); 
...
<div class="container">
		...
		<div class="col-lg-8 center-block" ng-controller='ProjCon'>
			<h1> Projekty <?php echo $_GET['projekt'];?></h1>
			<br/>
			...
			<div class="input-group">
				...
				<div class="list-group-item center-block" ng-repeat="projekt in projekty| filter : wyszukiarka">
					...
					<p> {{projekt.opisk}}</p>
					<a href="projekt.php?projekt={{projekt.id}}"><button type="button" class="btn btn-success  ">Wiecej</button></a>
				</div>
			</div>
		</div>
