	...
	mysqli_report(MYSQLI_REPORT_STRICT);
		...
		try 
		{
			...
			else
			{
				...
				$sql=mysql_query("SELECT * FROM `los`  ORDER BY imie"); 
		...
		<div class="col-lg-8 center-block" ng-controller='ProjCon'>
			...
			<input type="text" class="form-control" placeholder="Czego szukasz?" ng-model="wyszukiarka">
				...
				<table class="table table-striped table-bordered table-hover">
					<thead>
						<tr><td>imie</td> <td>obecność</td> <td>Funkcje</td></tr>
					</thead>
					<tbody>
						<tr ng-repeat="los in losowanie| filter : wyszukiarka">
					 <td>{{los.imie}}</td><td>{{los.los}}</td>
						<td><a href="losowanieobecnosc.php?o={{los.id}}"><button type="button" class="btn btn-success  ">obecny</button></a></td>
						</tr>
					</tbody>
				</table>
