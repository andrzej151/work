...
//	}
	...
	public void podzialproporcjonalny()
	 ...
	 public void czestotliwosc()
	 {
			...
			while(zakoncz)
			{
				...
				for(Procesy p:lprocesow)
				{
					if(!p.zakonczony())
					{
						...
						p.next();
						dysk.wyswielukladramek();
						if(p.getczestotliwosc()<dol)
						{
							nowypodzialcz(p, -1);
						}
						else
						{
							if(p.getczestotliwosc()>gora)
							{
								nowypodzialcz(p, 1);
							}
						}
	  ...
	  private void nowypodzialcz(Procesy r,int kier) {
		System.out.println("proces "+r.getnazwa()+" zglosil potrzebe zmianyilosci pam");
		if(kier==1)
		{
		
			r.setpriorytetST(r.getpriorytet()+1);
		}
		if(kier==-1)
		{	
			if(r.getpriorytet()>2)
		...
		dysk.nowypodzialpamiecifizycznej(nowypodzial);
		System.out.println("nowy podzzial ramek");
		dysk.wyswielukladramek();
